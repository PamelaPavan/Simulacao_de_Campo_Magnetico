# Com variação de campo e revestimento isolante
...
def visualizar_frame(ax, X, Y, Ex_total, Ey_total, V_potencial, magnitude_E, mascara_condutor, mascara_isolante_total, config_sim):
    ...
    isolante_rect = Rectangle((isolante_x_min, isolante_y_min),
                              ...
                              label=f'Isolante ($\epsilon_r = {constante_dieletrica_isolante}$)')
    ...
    if tipo_material == 'condutor':
        ...
        if abs(E_externo[0]) > 1e-6:
            ...
            ax.text(condutor_x_min - carga_offset_visual, y_centro_cond, f"$\sigma \propto {valor_carga:.1f}$",
            ...
            ax.text(condutor_x_max + carga_offset_visual, y_centro_cond, f"{sinal_dir}",
                    color='red', fontsize=tamanho_carga_visual, ha='center', va='bottom', fontweight='bold', zorder=10)
            ax.text(condutor_x_max + carga_offset_visual, y_centro_cond, f"$\sigma \propto {valor_carga:.1f}$",
                    color='black', fontsize=tamanho_valor_visual, ha='center', va='top', zorder=10)
        ...
        if abs(E_externo[1]) > 1e-6:
            ...
            ax.text(x_centro_cond, condutor_y_min - carga_offset_visual * 1.8 , f"$\sigma \propto {valor_carga:.1f}$",
            ...
            ax.text(x_centro_cond, condutor_y_max + carga_offset_visual, f"{sinal_sup}",
                    color='red', fontsize=tamanho_carga_visual, ha='center', va='center', fontweight='bold', zorder=10)
            ax.text(x_centro_cond, condutor_y_max + carga_offset_visual * 1.8 , f"$\sigma \propto {valor_carga:.1f}$",
                    color='black', fontsize=tamanho_valor_visual, ha='center', va='center', zorder=10)
    ...
    # Legenda
    legend_elements = [Rectangle((0, 0), 1, 1, color='lightblue', label=f'Isolante ($\epsilon_r = {constante_dieletrica_isolante}$)'),
...
# --- Cria animação ---
anim = animation.FuncAnimation(fig, atualizar, frames=len(valores_Ex), interval=200)

# --- Salva como GIF ---
anim.save('campo_condutor_revestido.gif', writer='pillow', fps=5)

print("GIF gerado com sucesso: campo_condutor_revestido.gif")
